...
/////////////////////////////////////////////////////
/////////////////////////////////////////////////////
extern "C" void axpy_gpu_st(int N, float ALPHA, float * X, int INCX, float * Y, int INCY)
{
    axpy_gpu_offset(N, ALPHA, X, 0, INCX, Y, 0, INCY);
    //printf("%f ",X[0]);
}

extern "C" void axpy_gpu_offset_st(int N, float ALPHA, float * X, int OFFX, int INCX, float * Y, int OFFY, int INCY)
{   
    //testmake
    //printf("%lf", N);
    //printf("xl");
...
__global__ void axpy_kernel_st(int N, float ALPHA, float *X, int OFFX, int INCX,  float *Y, int OFFY, int INCY)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i < N && i > 8) Y[OFFY+i*INCY] += ALPHA*X[OFFX+i*INCX];
    //printf("%f ",X[0]);
}
